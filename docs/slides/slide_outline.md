...
## Slide 2-3: VSCode セカンダリサイドバーでターミナル利用
- 見出し: VSCode セカンダリサイドバーでターミナル利用
- 本文:
  - Claude Code拡張をセカンダリサイドバー（右側）に配置
  - ドラッグ or 右クリック → Move to Secondary Side Bar
  - Cmd+Opt+B で表示/非表示を瞬時に切り替え
  - ターミナルベースの操作が可能（CLIの全機能を利用）
  - レイアウト: ファイルツリー(左) | エディタ(中央) | Claude Code(右)

## Slide 2-4: tmux活用のすすめ
- 見出し: tmux活用のすすめ
- 本文:
  - セッション永続化 — SSH切断・ターミナル閉じても作業継続
  - ペイン分割で複数のClaude Codeを並列稼働
  - バックグラウンド実行に最適（離席中も作業継続）
  - Agent Teamsの各エージェントをSplit Panesで表示可能
  - tmux基本操作: Ctrl+B → % or " でペイン分割

## Slide 2-5: ctコマンド — 独自実装でさらに便利に
- 見出し: ctコマンド — 独自実装でさらに便利に
- 本文:
  - ctはtmux + Claude Codeの独自統合ラッパー（自前実装例）
  - ct "タスクの説明" でプロンプト付き起動
  - --dangerously-skip-permissions を自動付与（サンドボックス併用必須）
  - セッション管理の自動化（名前付き・永続化）
  - おすすめ: 自分のワークフローに合わせてカスタム実装しよう

## Slide 2-6: 初回起動と /init
- 見出し: /init でプロジェクト設定
- 本文:
...
- 本文:
  ...
  - スクリプトで一括設定（共存環境を自動構築）
  - .cursor/rules → .claude/rules からの参照も可能
  - 両方のツールで同じプロジェクトルールを共有

## Slide 3-5: AGENTS.md — 複数AIツール共存の新標準
- 見出し: AGENTS.md — 複数AIツール共存の新標準
- 本文:
  - AGENTS.md: AIエージェント向けオープン標準（Linux Foundation管理）
  - Cursor / GitHub Copilot / OpenAI Codex 等が対応
  - Claude Codeは未ネイティブ対応 → @importで利用可能
  - CLAUDE.mdに @AGENTS.md と1行追加するだけ
  - メリット: 1ファイルで複数AIツールのルールを共通管理
...
## Slide 4-3: 実例 — シェル演算子禁止ルール
- 見出し: 実例: シェル演算子禁止ルール（CLAUDE.mdで対応）
- 本文:
...
## Slide 4-5: --dangerously-skip-permissions
...
## Slide 5-3: Slack連携 — リモート操作の起点
- 見出し: 実例: Slack連携 — リモート操作の起点
- 本文:
  - Stopイベント → Slack通知 → 3つの操作方法
  - 方法1: リアクションで次の指示を自動変換
  - 方法2: Remote Controlでセッションに直接接続
  - 方法3: @Claude でSlackから直接コーディング委任
  - Slack通知が全ての遠隔操作のトリガーとして機能

## Slide 5-4: Remote Control — スマホからClaude Codeを操作
- 見出し: Remote Control — スマホからClaude Codeを操作
- 本文:
  - ローカルPCのセッションをスマホ/ブラウザから遠隔操作
  - claude remote-control で起動、QRコードでスマホ接続
  - セッション中は /rc で切り替え可能
  - コード実行はすべてローカルPC上（安全）
  - Max プラン限定（Research Preview）

## Slide 5-5: Lint自動チェック
...
## Slide 7-2: Max 5x/20x — 制限倍率の注意点
- 見出し: Max 5x/20x — 制限倍率の注意点
- 本文:
  - 「5倍」「20倍」は目安。正確な比例ではない
  - 5時間ウィンドウ: おおむね倍率通り
  - 週間Sonnet枠: 実質3.5〜6倍にばらつき
  - Opus独立キャップ: Max 5xで15〜35h/週と厳しい
  - Opusコンピュートコスト: Sonnetの約5倍
  - 利用パターン・サーバー負荷で動的に変動する
...
- 本文:
  ...
  - スクリプトでCLAUDE.mdをSource of Truthに一括設定
  - /init コマンドで初期生成→既存ルールをマージ

## Slide 10-2: タイムリープ戦略 — /resume & /rewindで巻き戻し
- 見出し: タイムリープ戦略 — /resume & /rewindで巻き戻し
- 本文:
  - /resume: 過去セッションの会話を復元して再開
  - /rewind (Esc+Esc): コードと会話を特定時点に巻き戻し
  - --fork-session: 元を保護しつつ別アプローチを試す
  - 重要: Bash操作（git push, rm等）は巻き戻せない
  - Write/Edit経由の変更のみチェックポイント対象
...
## Slide 11-1: macOS通知
- 見出し: macOS通知 — タスク完了を見逃さない
- 本文:
  - タスク完了時にmacOSネイティブ通知を表示
  - 通知クリックでVSCodeを開く
  - 通知スクリプト: プロンプト記録（SQLite）+ 通知送信
