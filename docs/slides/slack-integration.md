...
sequenceDiagram
    ...
    participant Hook as slack-feedback.sh
    participant Slack as Slack API
    participant User as ユーザー（スマホ）

    CC->>Hook: Stop イベント発火
    Hook->>Hook: Keychainからトークン取得
    Hook->>Slack: chat.postMessage（完了通知）
    Slack-->>User: プッシュ通知

    loop フィードバック待機（1分→3分→5分）
        Hook->>Slack: reactions.get（リアクション確認）
        Slack-->>Hook: リアクション一覧
    end

    User->>Slack: 👍 リアクション追加
    Slack-->>Hook: リアクション検出
    Hook->>CC: "コミット&プッシュを実行"（指示変換）
```
...
タスク完了時にSlackに自動投稿される通知は以下のような形式です。

```
┌─ #kai-cursor-times ─────────────────────────────────────────┐
│                                                              │
│  🤖 Claude Code Bot                           14:32         │
│  ┌──────────────────────────────────────────────────────┐   │
│  │ [claude-code-guide] タスク完了 (2026-03-03 14:32:15) │   │
│  │                                                      │   │
│  │ 変更内容:                                            │   │
│  │ - docs/vscode-guide.md を更新                        │   │
│  │ - 3ファイル変更、+152/-48行                          │   │
│  │                                                      │   │
│  │ フィードバックをお待ちしています...                   │   │
│  └──────────────────────────────────────────────────────┘   │
...
│                                                              │
│  ┌ スレッド返信 ────────────────────────────────────────┐   │
│  │ 👤 kai: テストも実行してから CP お願いします         │   │
│  └──────────────────────────────────────────────────────┘   │
│                                                              │
└──────────────────────────────────────────────────────────────┘
```

### 3-3a. リアクション→指示変換マッピング
...
タスク完了時にmacOSネイティブ通知を表示し、通知クリックでVSCodeを開きます。通知スクリプトを作成してHookイベントと連携させます。
...
通知スクリプトでは以下の2つの機能を実装します:

1. **プロンプト記録（SQLite）**: UserPromptSubmitイベントでプロンプトをDBに記録
2. **通知送信**: Stopイベントでterminal-notifierによるmacOS通知を送信
...
DB_PATH = "~/.claude/notifications/prompts.db"
...
```json
{
  "hooks": {
    "UserPromptSubmit": {
      "type": "command",
      "command": "python3 ~/.claude/notifications/notify.py UserPromptSubmit"
    },
    "Stop": {
      "type": "command",
      "command": "python3 ~/.claude/notifications/notify.py Stop"
    }
  }
}
```
...
## 8-5. Slack通知からのリモート操作 — 3つの方法

Slack通知を受信した後、PCに戻らずに遠隔で操作する方法が3つあります:
...
### 方法2: Remote Control — スマホからセッションに直接接続

ローカルPCのClaude Codeセッションをスマホやブラウザから遠隔操作できます。
...
SlackのClaude Code連携を使って、Slackから直接コーディングタスクを委任できます。

- `@Claude` でメンションしてタスクを指示
- GitHub連携でPR作成・コードレビューまで自動実行
- 非同期でコーディングを進められる

**使い分けの目安**:
- 定型操作（コミット、テスト等）→ 方法1（リアクション）
- 対話的な操作が必要 → 方法2（Remote Control）
- 新しいタスクを非同期で依頼 → 方法3（@Claude）
...
```mermaid
graph TD
    A["Claude Code: Edit実行"] -->|PostToolUse| B["Hook: Lint チェック"]
    B -->|エラー検出| C["Claude Code: 自動修正 (Edit)"]
    C -->|PostToolUse| D["Hook: Lint チェック (再帰!)"]
    D -->|"ロックファイル存在?"| E{再帰判定}
    E -->|YES| F["exit 0 でスキップ<br>(ループ防止)"]
    E -->|NO| G["Lint 実行<br>(正常処理)"]
