...
MCP（Model Context Protocol）は、Claude Codeと外部ツール・サービスを接続するためのオープンスタンダードです。MCPを設定すると、Claude CodeからSlackへの投稿、GitHubのissue操作、Notionのページ更新などを自然言語で指示するだけで実行できるようになります。このガイドでは、MCPの仕組みと設定方法を解説します。
...
Anthropicが検証していないサードパーティMCPサーバーは自己責任で使用してください。悪意のあるサーバーがプロンプトインジェクションを行う可能性があります。

### projectスコープの承認フロー

`.mcp.json`（projectスコープ）に定義されたMCPサーバーは、初回接続時にユーザーの承認が求められます。リポジトリをcloneした後に見覚えのないMCPサーバーが表示された場合は慎重に判断してください。
