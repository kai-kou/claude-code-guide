...
Claude Codeでは、繰り返し使うワークフローを「スキル」として、専門的なタスクを担う独立したAIを「エージェント」として定義できます。これらを活用することで、チーム固有のベストプラクティスをClaude Codeに組み込み、開発効率を大幅に向上させることができます。
...
| 観点 | スキル | エージェント |
|------|--------|------------|
| **位置づけ** | 再利用可能なプロンプト | 専門家としての役割定義 |
| **コンテキスト** | メインセッションで実行（`context: fork`で分離も可） | 常に独立したコンテキスト |
| **呼び出し** | `/コマンド名` またはClaude自動 | Task toolでデリゲート |
| **適したケース** | 定型ワークフロー（デプロイ、レビュー等） | 専門的なサブタスク（調査、テスト等） |
...
|------|--------------|-------------|
| **コンテキスト** | 独自ウィンドウ。結果を呼び出し元に返す | 完全独立。双方向通信可能 |
| **通信** | メインエージェントへの報告のみ | チームメイト同士が直接通信 |
| **調整** | メインが全管理 | 共有タスクリストで自己調整 |
| **コスト** | 低い | チームメイト数に比例して増加 |
| **向き** | 結果だけが重要な集中タスク | 議論・協調が必要な複雑作業 |
