...
- ノート: コード補完ツールではなくエージェント。タスク委譲が基本

## Slide 1-2: Cursorとの違い
- 見出し: Cursor vs Claude Code — 設計思想の違い
- 本文:
  - Cursor: IDE-first（VS Code fork）、ユーザーが操縦しAIがアシスト
  - Claude Code: Agent-first（CLIが中心）、タスクを委譲しAIが自律実行
  - Cursor強み: リアルタイムTab補完、インライン編集
  - Claude Code強み: 大規模リファクタ、MCP連携、Hooks自動化、Agent Teams
  - 結論: 両方を併用するのが最も効率的

## Slide 1-3: 対応モデル
- 見出し: 3つのモデルと使い分け
- 本文:
  - Opus 4.6: 最高性能。アーキテクチャ判断、難解なデバッグ
  - Sonnet 4.6: 速度と知性のバランス。日常のコーディング（推奨）
  - Haiku 4.5: 最速・軽量。大量ファイル処理、コスト節約
...
- 本文:
  ...
  - その他: brew install --cask claude-code / npm install -g（手動更新）
...
- 本文:
  ...
  - Cmd+Opt+B でサイドバー表示切替
  - レイアウト: ファイルツリー(左) | エディタ(中央) | Claude Code(右)
- ノート: ターミナルとエディタを切り替えずに作業可能
...
- 本文:
  ...
  - 3つが自動的にマージされて読み込まれる
  - 活用例:
    - グローバル: キャラ設定、Git操作ルール
    - ワークスペース: デグレ防止、コミット規律
    - プロジェクト: 技術スタック、API設計方針
...
- 本文:
  ...
  - OK: mkdir -p /path → git init /path を順次実行
  - cat file | grep → Grepツールを使用
  - echo "text" > file → Writeツールを使用
- ノート: CLAUDE.mdで明文化することで毎回のユーザー確認を回避
...
- 本文:
  ...
  - 使用条件（すべて満たすこと）:
    ...
    3. tmux環境などバックグラウンド実行が必要
  - ctコマンドは自動付与（サンドボックス併用前提）
...
- 本文:
  ...
  - 他にも: UserPromptSubmit, SubagentStop, PreCompact 等
...
## Slide 6-2: ステータスラインの読み方
- 見出し: 4行ダッシュボードの読み方
- 本文:
  - Line 1: プロジェクト | モデル | コンテキスト使用率 | コスト | 時間
  - Line 2: コンテキストウィンドウ詳細（現在/最大, W:キャッシュ書込, R:読取）
  - Line 3: 利用制限（⚡5h | 📅7d | 🎵Sonnet | 🎹Opus）
  - Line 4: メモリ使用量（CLI | App | VSCode）

## Slide 6-3: /compact の活用
- 見出し: /compact — コンテキスト圧縮
- 本文:
  - 情報を要約・圧縮して新しいセッションに引き継ぐ
  - 使い方: /compact 保持すべき情報を指示で明示
  - 例: /compact API変更に集中、UI議論は破棄
  - 情報ロス対策:
    - compact-reinject.sh で自動再注入
    - CLAUDE.mdに保持ルールを記述
    - タスク管理を外部ファイル（scrum/tasks.md）で管理
...
- 本文:
  ...
  - 📅 7日間制限 + リセットまでの時間
  - 🎵 Sonnet使用率 / 🎹 Opus使用率
  - Usage API: ~/.claude/cache/limit-tracker.json にキャッシュ
...
## Slide 8-1: ステータスラインとは
- 見出し: ステータスライン — リアルタイム監視
- 本文:
  - settings.json の statusLine 設定で有効化
  - カスタムスクリプト（statusline-command.sh）で情報収集
  - VSCode下部のステータスバーに表示
  - 情報源: セッション、コンテキスト、制限、メモリ
...
- 本文:
  ...
  - Line 3: ⚡ 42% 3h12m | 📅 18% 5d14h | 🎵 38% | 🎹 15% | 🔥 Moderate
...
- 本文:
  - コンテキスト使用率:
    - 緑(〜60% OK): 安全、作業継続
    - 黄(60-80% WATCH): 注意
    - 赤(80%〜 SWITCH): /compact または /clear 推奨
  - 利用制限:
    - 緑(〜70%): 安全
    - 黄(70-85%): ペースに注意
    - 赤(85%〜): モデル切替 or セッション分割
  - 制限影響度: Low → Moderate → High → Critical
...
- 本文:
  ...
  - 3つのトランスポート: http（推奨）、stdio（ローカル）、sse（非推奨）
  - Claude Code以外にもCursor等で採用拡大中
...
- 見出し: 画像の扱い方
- 本文:
  - Cursor: チャットに画像貼り付け（一度に1枚、セッション終了で消失）
  - Claude Code: Read toolでファイルパスを直接指定
  - ペーストキャッシュ: Cmd+V → /tmp/claude/paste-*.png に自動保存
...
- 本文:
  ...
  - 前提: brew install terminal-notifier
...
## Slide 13-2: ベストプラクティス10選
- 見出し: ベストプラクティス10選
- 本文:
  1. Plan Modeで大きなタスクは計画してから実行
  2. /clear をタスク完了ごとに実行（コンテキスト管理）
  3. CLAUDE.mdに検証基準を明記
  4. 1コマンド=1Bash呼び出し（シェル演算子回避）
  5. サンドボックスを有効にして安全に運用
  6. Hooksで手作業を自動化（Lint・通知・保護）
  7. キャッシュヒット率を上げてコスト・制限を節約
  8. Haiku で軽作業を処理して制限を温存
  9. MCPで外部ツールと連携して作業範囲を拡大
  10. タスク管理を外部ファイルで管理（compact耐性）
...
- 本文:
  ...
  - Hooks ドキュメント: https://code.claude.com/docs/en/hooks
  - MCP ドキュメント: https://code.claude.com/docs/en/mcp
