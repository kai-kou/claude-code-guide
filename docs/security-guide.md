...
```mermaid
flowchart TD
    A["ツール呼び出し<br>例: Bash(git push)"] --> B{Denyリストに<br>マッチ?}
    B -->|YES| C["🚫 ブロック<br>(deny は常に最優先)"]
    B -->|NO| D{Allowリストに<br>マッチ?}
    D -->|YES| E["✅ 自動実行<br>(確認なし)"]
    D -->|NO| F{シェル演算子<br>を含む?}
    F -->|YES| G["⚠️ パターンマッチ不可<br>常に確認プロンプト表示"]
    F -->|NO| H["❓ 確認プロンプト表示<br>(defaultMode に依存)"]
...
```
  Claude Code が .env を Edit しようとした場合:
  ...
  └────────────────────────┬────────────────────────────────┘
                           ...
                           ▼
  ┌─────────────────────────────────────────────────────────┐
  │  第1防衛線: settings.json permissions.deny              │
  │  "deny": ["Edit(.env)", "Write(.env)"]                 │
  │                                                         │
  │  ─── Claude Code 本体が直接参照 ─── 最も堅牢 ───       │
  └────────────────────────┬────────────────────────────────┘
                           │ ブロック! (deny優先)
                           ▼
                      ╔═════════╗
  ...
  万が一 Deny リストに漏れがあった場合:
                           │
                           ▼
  ┌─────────────────────────────────────────────────────────┐
  │  第2防衛線: PreToolUse Hook                             │
  │  protect-sensitive-files.sh                             │
  │                                                         │
  │  ─── ファイルパターンで判定 ─── exit 2 でブロック ───   │
  └────────────────────────┬────────────────────────────────┘
                           │ ブロック! (exit 2)
                           ▼
                      ╔═════════╗
...
- [Claude Code公式 - 権限管理](https://docs.anthropic.com/en/docs/claude-code/security)
- [Claude Code公式 - settings.json設定](https://docs.anthropic.com/en/docs/claude-code/settings)
- [サンドボックス設定ガイド](https://docs.anthropic.com/en/docs/claude-code/security#sandbox)
